...
public class SgfParsingTest {
    ...
    public void threeNodes() throws SgfParsingException {
        String input = "(;A[B];B[C];D[E])";
        SgfNode expected = new SgfNode(Map.of("A", List.of("B")),
                List.of(
                        new SgfNode(Map.of("B",
                                List.of(
                                        "C")),
                                List.of(
                                        new SgfNode(Map.of("D", List.of("E")))
                                ))
                ));
        SgfNode actual = new SgfParsing().parse(input);
        assertEquals(expected, actual);
    ...
    public void originalExample() throws SgfParsingException {
        String input = "(;FF[4]C[root]SZ[19];B[aa];W[ab])";
        SgfNode expected = new SgfNode(Map.of("FF", List.of("4"),
                "C", List.of("root"),
                "SZ", List.of("19")),
                List.of(
                        new SgfNode(Map.of("B",
                                List.of(
                                        "aa")),
                                List.of(
                                        new SgfNode(Map.of("W", List.of("ab")))
                                ))
                ));
        SgfNode actual = new SgfParsing().parse(input);
        assertEquals(expected, actual);
